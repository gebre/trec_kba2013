...
public class FeatureExtractor extends Configured implements Tool {
	...
	private static final Logger LOG = Logger
			.getLogger(FeatureExtractor.class);
	...
	public static class MyMapper extends
		...
		public void setup(Context context) throws IOException,
			...
			Annotation = loadAnnotation(ANNOFILEPATH_HDFS);
			 ...
			 System.out.println("MyVer's contents:\n");
			 
			 	for(String k: MyVer.keySet()){
		...
		public void map(Text key, StreamItemWritable value, Context context)
			...
			*/
			...
			String title_body_anchor = body +" "+ title +" " + anchor;
			//if(streamid.equalsIgnoreCase("1320240072-21c9cd2711170499ffccab5ddd0b0467")){
				//context.write(new Text(streamid), new Text(title_body_anchor) );
			//}
			
			//if(MyVer.containsKey(streamid)){
				//context.write(new Text(streamid), new Text(title_body_anchor) );
...
//				hmm.add("1319530036-3749b03679cfb85fd21ed7d842813e98n");
//				hmm.add("1319471526-5bc708945e67cbe7ad0e6ba6945cf1c5n");
//				
//				if(hmm.contains(streamid)){
//					String st = stream_doc_id.toString()+DbPedia.get(topic);
//					context.write(new Text("======================================================================="+st+"\n"), new Text(title_body_anchor+"\n==============================================================\n") );
//				}
//				
//				System.out.println("what does my ver has" +MyVer);
//				if (Annotation.containsKey(stream_doc_id.toString())) {
//					if(MyVer.containsKey(streamid.toString())){
//						context.write(new Text(streamid), new Text(title_body_anchor) );
//					}
//				}
				if (Annotation.containsKey(stream_doc_id.toString())) {	
					...
					//all_attr.put("Class",  new Integer(Annotation.get(stream_doc_id.toString()).intValue()).toString());
					all_attr.put("Class", new Integer(anno_value).toString());
					...
					if (containsMention(title_body_anchor,
						...
						// if (containsMention(body+title+anchor, new
						// HashSet<String> (Arrays.asList(topic.replaceAll("_",
						// " ")))) == 1) {
						...
						temp = temp * scale;
						all_attr.put("gcld", new Double(temp).toString());
						temp = 0.0;
						...
						if (ContextFreqsL.containsKey(topic)) {
							for (String contxL_key : ContextFreqsL.get(topic)
									.keySet()) {
								if (containsMention(
										title_body_anchor,
										new HashSet<String>(Arrays
												.asList(contxL_key))) == 1) {
									temp = temp
											+ ContextFreqsL.get(topic).get(
													contxL_key);
								}

							}
							temp = temp * scale;
							all_attr.put("contxL", new Double(temp).toString());
							
							temp = 0.0;
						}
						if (ContextFreqsR.containsKey(topic)) {
							for (String contxR_key : ContextFreqsR.get(topic)
									.keySet()) {
								if (containsMention(
										title_body_anchor,
										new HashSet<String>(Arrays
												.asList(contxR_key))) == 1) {
									temp = temp
											+ ContextFreqsR.get(topic).get(
													contxR_key);
								}

							}
							temp = temp * scale;
							all_attr.put("contxR", new Double(temp).toString());
							temp = 0.0;
						}
						...
						wiki_map = ReturnFreqs(WikiContent.get(topic));
		...
		private void loadContext(String xcontext) {
			...
			try {
				...
				while ((strLine = br.readLine()) != null) {
					...
					/*
					String temp_str1 = str[1];
					String temp_str2 = str[2];
					*/
		...
		private void extractNgrmFrequency() {
	    	...
	    	for(String k: ContextFreqsR.keySet()){
	    		List <Integer> l = new ArrayList <Integer> (ContextFreqsR.get(k).values());
	    		Collections.sort(l);
	    		int leng =l.size();
	    	...
	    	for(String k: ContextFreqsL.keySet()){
	    		List <Integer> l = new ArrayList <Integer> (ContextFreqsL.get(k).values());
	    		Collections.sort(l);
	    		int leng =l.size();
	    		List <Integer> top_values = l.subList(leng-top_n, leng);
	    		HashMap <String, Integer>hs = new HashMap<String, Integer>();
	...
	public int run(String[] args) throws Exception {
		...
		for (int i = 0; i < args.length; ++i) {
			try {
				...
				} else if ("-s".equals(args[i])) { myverFile = args[++i];
